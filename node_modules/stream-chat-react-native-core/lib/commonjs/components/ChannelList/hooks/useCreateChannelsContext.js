...
var useCreateChannelsContext = exports.useCreateChannelsContext = function useCreateChannelsContext(_ref) {
  ...
  var channelValueString = channels == null ? void 0 : channels.map(function (channel) {
    ...
    return "".concat((_channel$data$name = (_channel$data = channel.data) == null ? void 0 : _channel$data.name) != null ? _channel$data$name : '').concat((_channel$id = channel.id) != null ? _channel$id : '').concat((_channel$state$unread = channel == null || (_channel$state = channel.state) == null ? void 0 : _channel$state.unreadCount) != null ? _channel$state$unread : '').concat(Object.values(channel.state.members).map(function (member) {
      var _member$user;
  ...
  }, [channelValueString, error, forceUpdate, hasNextPage, loadingChannels, loadingNextPage, channelListInitialized, refreshing]);
